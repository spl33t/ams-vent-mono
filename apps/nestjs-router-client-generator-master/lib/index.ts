...
import { ClassDeclaration, InterfaceDeclaration, Project, SourceFile, SyntaxKind, TypeReferenceNode } from "ts-morph";
import { analyzeController } from "./controller";
import { stringifyLog } from "./logging";
...
export function run(directory: string, extension: string) {
    ...
    controllerPaths.forEach(filePath => {
        const controller = analyzeController(project, filePath)
        stringifyLog(controller)
...
const directoryPath = path.resolve("../api");
const fileExtension = '.controller.ts';

run("C:/Users/spl33/Desktop/andrey-react/apps/api/src", fileExtension);
